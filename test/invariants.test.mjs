...
test("INV7: rules/detect.md が多層検出・3分類・安全の実質を持つ（字面でなく規定内容）", () => {
  ...
  assert.ok(/1項目が複数箇所へ一致する一括置換は禁止/.test(t), "裸の語・複数箇所一致による一括置換を禁止する");
  ...
  assert.ok(/全出現の判断が確定するまで次の語や適用へ進まない/.test(t), "全出現の判断を漏れなく確定する");
  ...
  assert.ok(/承認されたグループを箇所単位の辞書項目へ展開したあとの別アクション/.test(t), "意味グループの承認を箇所単位へ展開してから適用する");
  assert.ok(/複数語のまとめ承認は成立しない/.test(t), "複数語をまたぐまとめ承認は禁止する");
...
test("INV7: rules/workflow.md が一巡と安全の前提を持ち、CLI と LLM の役割分担を規定する", () => {
  ...
  assert.ok(/意味と判断が同じ出現グループごとに提示/.test(t), "意味同値の出現はグループで提示する");
  assert.ok(/グループ内の全ファイル・行/.test(t), "グループの全対象を列挙する");
  assert.ok(/裸の語を `from` にした置換[\s\S]*複数箇所に当たる置換は `apply` が書き込み前に拒否/.test(t), "箇所単位で適用する");
  assert.ok(/未判断の語が残れば完了にしない/.test(t), "再検査で未判断の残存を確認する");
  assert.ok(/意味を保てる理由/.test(t), "workflowでも意味保存の確認を必須にする");
  assert.ok(/書き換え・維持・別案・グループ分割を判断/.test(t), "利用者がグループの判断や境界を修正できる");
