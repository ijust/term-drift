...
test("INV7: rules/detect.md が多層検出・3分類・安全の実質を持つ（字面でなく規定内容）", () => {
  ...
  assert.ok(/同じ箇所を書き換えた完成文/.test(t), "候補には文脈内の置き換え結果が必須");
  ...
  assert.ok(/各箇所を人が1箇所ずつ承認したあとの別アクション/.test(t), "適用は箇所別判断の人承認後に行う");
...
test("INV7: rules/workflow.md が一巡と安全の前提を持ち、CLI と LLM の役割分担を規定する", () => {
  ...
  assert.ok(/その1箇所について書き換え・維持・別案を判断/.test(t), "利用者が箇所ごとに判断を修正できる");
