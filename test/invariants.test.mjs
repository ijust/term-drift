...
test("INV7: rules/detect.md が多層検出・3分類・安全の実質を持つ（字面でなく規定内容）", () => {
  ...
  assert.ok(t.includes("複数箇所へ一致する項目") && t.includes("部分交差する書き換え単位は禁止"), "複数箇所一致と重なる書き換え単位を禁止する");
  assert.ok(/以前の承認を流用しな/.test(t), "未提示の出現には以前の承認を流用しない");
...
test("INV7: rules/workflow.md が一巡と安全の前提を持ち、CLI と LLM の役割分担を規定する", () => {
  ...
  assert.ok(/書き換え・維持・保留・別案・グループ分割を判断/.test(t), "利用者がグループの判断や境界を修正できる");
