...
test("全出現を個別検査し、意味同値だけをグループ化して未提示箇所への承認流用を拒否する", () => {
  ...
  assert.match(skill, /Expand every approved replacement group into non-overlapping rewrite-unit JSON dictionary items/);
...
test("意味検査の厚さを保ったまま通常の判断画面と承認後の遷移を軽くする", () => {
  ...
  assert.match(workflow, /通常の判断面は、グループ内の全ファイル・行、各引用、置換対象の各完成文、同じ判断にまとめられる理由1文/);
...
test("全出現を検査したまま重ならない書き換え単位へ展開する", () => {
  ...
  assert.match(skill, /Legacy dictionaries without `term_occurrences` remain readable/);
  assert.match(workflow, /検査は出現ごとのまま保ち/);
  assert.match(workflow, /重複・包含・部分交差する書き換え単位/);
  assert.match(detect, /`term_occurrences` は正の整数/);
  assert.match(detect, /別の未決候補語を同じ項目へ含めない/);
});
