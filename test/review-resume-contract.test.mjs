...
test("未追跡ファイルは対象列挙と復元リスクの明示確認後だけ書き換える", () => {
  assert.match(skill, /list every untracked target path[^]*Git cannot restore its current contents/);
  assert.match(skill, /General wording approval or delegated review authority does not count/);
  assert.match(skill, /apply --allow-untracked/);
  assert.match(workflow, /該当する全パスを列挙[^]*Git から復元できない[^]*明示的に確認/);
  assert.match(workflow, /文言の承認やレビュー判断の委任だけでは、この復元リスクへの確認を兼ねない/);
  assert.match(workflow, /apply --allow-untracked/);
});
