...
2. Expand every approved replacement group into non-overlapping rewrite-unit JSON dictionary items. Continue to inspect and enumerate every occurrence individually, but when multiple occurrences of the current term are inseparable inside one approved passage rewrite, use one item for that passage. Every new item has `term`, positive integer `term_occurrences`, the repository-relative `path`, and a `from` passage that occurs exactly once in that file. `term_occurrences` must equal the number of prose occurrences of `term` inside `from`; use `1` for a single occurrence. Never combine different unresolved terms in one item, use the bare suspect term as `from`, omit `path`, overlap rewrite units, or reuse one item across matching files or lines. Legacy dictionaries without `term_occurrences` remain readable.
