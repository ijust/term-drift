...
2. Re-read the pinned rules and refresh `scan`, `ledger`, and the full literal-occurrence search as internal read-only preparation. Do not ask the user for term-drift permission to perform each preparation step, choose a CLI command, or confirm an installation already evidenced by the project. Obey any security approval that the execution environment itself requires; that is not a term-drift decision.
...
5. If the exact next unresolved point cannot be established from the available evidence, state the restored decisions and the missing evidence once, briefly, and ask only which safe review point to resume from.
...
3. In guided review, summarize approved rewrites before writing. In delegated review, retain an auditable occurrence-level summary and report it at a useful checkpoint or handoff. Prepare to apply decided units only.
4. Check whether any decided target is untracked. If none are, run the pinned `apply <dictionary> <target>` command. Otherwise, list every untracked target path and warn that Git cannot restore its current contents. Obtain explicit confirmation for those paths unless the user's current instruction already explicitly authorizes rewriting untracked files or the named untracked paths. General wording approval or delegated review authority does not count as this recovery-risk confirmation. After confirmation, run the pinned `apply --allow-untracked <dictionary> <target>` command; never use the flag for an unlisted or unconfirmed path.
5. Report changed and skipped files together with each change's `decisionSource`, `decidedAt`, and `delegationScope`. Report untracked changes from `warningsUntracked` as not recoverable through Git. A tracked file with uncommitted changes may be updated when the exact `from` passage still matches uniquely; report the CLI's dirty-file warning and preserve unrelated changes. Never bypass UTF-8, unique-match, overlap, protected-fragment, or repository-boundary safeguards.
...
7. Repeat the semantic detection pass from the loaded rules. Stop with either `指摘なし`, approved reason-bearing exceptions, or the next unresolved semantic group.
...
- Do not create a ledger, add an exception marker, or update ledger status without explicit human approval at the granularity defined above. Apply prose wording only through guided approval or an explicit, bounded delegation scope.
