...
Keep the decision surface compact by default. State term-level classification and counts once, at the start of the term. For later occurrences, do not repeat the term summary, prior decisions, progress narration, or the full semantic checklist. Show only the file and line, the short quote, the complete contextual rewrite, and one concise sentence explaining semantic preservation. Expand the explanation only when ambiguity or risk cannot be resolved in one sentence, or when the user asks for detail. Compact output compresses presentation only: the internal reading and semantic checks remain complete.
