...
6. **書き換えの適用** — 決定済みの置換だけを、同じ入力なら同じ結果になる処理でファイルへ適用する（未追跡ファイルは、Git で復元できないことを確認した後だけ許可する）
...
`apply` は、決定済みの項目だけを、指定された相対パス内の一意な 1 箇所へ適用します。新しい形式では、誰の判断で変更するかも検証します。適用結果には、変更箇所ごとの `from`、`to`、`decisionSource`、`decidedAt`、`delegationScope` が含まれます。パスのない辞書や、1 つの項目が複数箇所に一致する辞書は、書き込む前に拒否します。対象は git worktree 内の UTF-8 文書です。追跡済み文書にステージされていない変更があっても、現在の内容に `from` が一意に一致すれば、ほかの差分を残したまま適用し、`warningsDirty` と標準エラー出力で警告します。未追跡文書は既定では `skippedUntracked` となります。対象パスを確認し、現在内容を Git から復元できないことを理解したうえで明示的に許可する場合だけ、`apply --allow-untracked` で適用できます。この場合は `warningsUntracked` と標準エラー出力で警告します。非 UTF-8 のファイル、曖昧な一致、重なる置換、コードやリンク先など保護対象の変更は、引き続き拒否するかスキップします。
