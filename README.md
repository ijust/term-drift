...
候補は単語の一覧ではなく、全出現を内部で棚卸ししたうえで、実際に使われている1箇所の引用、出典、置き換え語の候補、置き換え後の文を順に提示します。人は一度に1箇所だけ、前後の意味と書き換え結果を見て承認・否認・保留を判断します。同じ文面が別ファイルにあっても承認は流用されません。通常の判断画面は引用・完成文・意味保存理由1文へ絞り、詳しい説明は曖昧な箇所か利用者が求めたときだけ表示します。

レビューの途中で「造語チェックの続き」と依頼した場合、skillは会話と承認済み記録から既決事項を復元し、操作手順を聞き直さず、全出現を内部棚卸しした次候補の未決1箇所から再開します。再開位置を証拠から確定できない場合だけ、復元内容と不足を短く示して安全な再開点を確認します。
...
5. **人承認** — 1語ずつ、さらに出現箇所を1箇所ずつ確認する（まとめ承認はできない）
